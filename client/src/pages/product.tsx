...
import LuxLoader from "@/components/ui/LuxLoader";
...
export default function Product() {
    ...
    // We will render the loader regardless, but only show content if product is available
    ...
    const allImages = product ? [product.imageUrl, product.imageUrl2, product.imageUrl3].filter(Boolean) as string[] : [];
    const mainImage = selectedImage || product?.imageUrl;
    // Filter out the current product from suggestions
    const suggestedProducts = suggestions?.filter(p => product ? p.id !== product.id : true).slice(0, 4) || [];

    return (
        <div className="min-h-screen bg-white">
            <LuxLoader isLoading={productLoading} text="جاري تحميل تفاصيل المنتج..." />

            {/* If product data is not loaded yet, don't try to render the rest of the page */}
            {product && (
                <>
                    <Navbar />
